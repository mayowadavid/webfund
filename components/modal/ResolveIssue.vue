<template>
  <v-modal :show="show" title="Issue" backdrop="static" @hide="hideModal">
    <v-form
      id="signup-form"
      ref="form"
    ...
    >
      ...
      <div class="form-group mb-5">
        <label for="input-description">Issue description</label>
        <div>
          <validation-provider
            v-slot="{ errors, classes }"
      ...
      <div class="form-group mb-5">
        <label for="input-assign_to">Status</label>
        <validation-provider
          v-slot="{ errors, classes }"
          name="user"
          rules="required"
      ...
      <template v-slot:footer>
        <div class="mt-8">
          <v-button :loading="loading" class="flex">Resolve Dispute</v-button>
        </div>
      </template>
    </v-form>
  </v-modal>
